...
export default function EarningsScreen() {
  ...
  const [timeframe, setTimeframe] = useState<"weekly" | "monthly" | "year">(
    "weekly",
  );
  ...
  const { selectedData, trendString, trendPositive } = useMemo(() => {
    ...
    const currentGross = currentFiltered.reduce(
      (sum, t) => sum + (t.final_price || t.budget || 0),
      0,
    );
    ...
    if (timeframe === "weekly") {
      ...
      previousFiltered = realTransactions.filter(
        (t) => t.date >= prevWeekStart && t.date < weekAgo,
      );
      ...
      previousFiltered = realTransactions.filter(
        (t) => t.date.getFullYear() === prevYear,
      );
    }

    const previousGross = previousFiltered.reduce(
      (sum, t) => sum + (t.final_price || t.budget || 0),
      0,
    );
      ...
      >
        ...
        <View style={styles.metricsRow}>
          <MetricCard
            ...
            title="Gross"
            value={formatZAR(selectedData.total)}
          />
          <MetricCard
            icon={<ListChecks size={18} color="#6366f1" />}
            title="Jobs"
            value={`${selectedData.jobs}`}
          />
          <MetricCard
          ...
          {realTransactions.length === 0 ? (
            <View style={styles.emptyCard}>
              ...
              <Text style={styles.emptySub}>
                Your payouts will appear here once jobs are completed.
              </Text>
            </View>
          ) : (
                ...
                <TouchableOpacity style={styles.transactionRow}>
                  ...
                  <View style={styles.transactionDetails}>
                    <Text style={styles.transactionTitle}>
                      {t.title || t.service || "Service Rendered"}
                    </Text>
                    <Text style={styles.transactionDate}>
                      {t.date.toLocaleDateString("en-ZA", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
                  ...
                  <View style={styles.amountArea}>
                    <Text style={styles.transactionAmount}>
                      {formatZAR(t.final_price || t.budget)}
                    </Text>

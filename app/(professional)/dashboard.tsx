...
console.log("!!! PROFESSIONAL INDEX.TSX LOADED SUCCESSFULLY !!!");
...
export default function ProfessionalDashboard() {
  ...
  const [selectedJobForBid, setSelectedJobForBid] = useState<Booking | null>(
    null,
  );
  ...
  const canPerformWork = () =>
    professional?.isVerified === true && isOnline === true;

  const renderJobSpecifics = () => {
    if (!selectedJobForBid?.propertyDetails || !selectedJobForBid?.subService)
      return null;
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "jobs"),
      where("assigned_professional_id", "==", userId),
    );
  ...
  const verifyAndStartJob = async () => {
    ...
    try {
      ...
      if (inputPin !== serverPin) {
        ...
        Alert.alert(
          "Invalid PIN",
          `Input "${inputPin}" does not match server.`,
        );
        return;
      }
  ...
  const metrics = useMemo(
    () => ({
      totalEarnings: bookings
        ...
        .reduce((s, b) => s + Number(b.price || b.budget || 0), 0),
      jobsCompleted: bookings.filter((b) => b.status === "completed").length,
      avgRating: professional?.rating || 0,
      upcomingJobs: bookings.filter((b) =>
        ["confirmed", "accepted", "assigned", "in_progress"].includes(
          b.status || "",
        ),
      ).length,
  ...
  const submitBid = async () => {
    ...
    try {
      ...
      if (result.data) {
        ...
        Alert.alert(
          "Success",
          "Your bid has been submitted to the marketplace!",
        );
      }
    } catch (error: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.userName}>
            {professional?.name?.split(" ")[0] || "Partner"}
          </Text>
        </View>
        <Image
          source={{
            uri:
              professional?.profileImage || "https://via.placeholder.com/100",
          }}
          style={styles.avatar}
        />
      </View>
      ...
      >
        ...
        <View style={styles.gridContainer}>
          <NavCard
            ...
            label="Earnings"
            value={formatZAR(metrics.totalEarnings)}
          ...
          <NavCard
            ...
            onPress={() => router.push("/upcoming-bookings")}
          />
          <NavCard
            icon={<Briefcase color="#6366f1" size={20} />}
            label="Equipment Hub"
            value="1"
            onPress={() => router.push("/Equipment/rental-marketplace")}
          />
        </View>
              ...
              <View key={b.id} style={styles.jobCard}>
                ...
                <View style={styles.locationRow}>
                  ...
                  <Text style={styles.locationText} numberOfLines={1}>
                    {b.location?.address || b.property_address}
                  </Text>
                </View>
                <View style={styles.jobFooter}>
                  ...
                  <View style={styles.btnRow}>
                    ...
                    >
                      <Text style={styles.subBtnText}>View</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                      <Text style={styles.mainBtnText}>
                        {b.status === "in_progress" ? "Ongoing" : "Start"}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </View>
        ...
        {availableJobs.slice(0, 15).map((job) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                styles.marketCard,
                hasAlreadyBid && styles.alreadyBidCard,
              ]}
              onPress={() => {
            ...
            >
              <View style={{ flex: 1 }}>
                <Text
                  style={[
                    styles.marketTitle,
                    hasAlreadyBid && { color: "#fff" },
                  ]}
                >
                  {job.title || job.subService}
                </Text>
                <View
                  style={[
                ...
                >
                  <Calendar
                    size={10}
                    color={hasAlreadyBid ? "#fff" : "#6366f1"}
                  />
                  <Text
                    style={[
                ...
                <Text
                  style={[
                    styles.marketMeta,
                    hasAlreadyBid && { color: "rgba(255,255,255,0.7)" },
                  ]}
                >
                  {job.location?.city || "Durban"}
                </Text>
              </View>
              <View style={{ alignItems: "flex-end" }}>
                ...
                >
                  {formatZAR(job.budget || job.customerInitialBid || 0)}
                </Text>
                <Text
                  style={[
                    styles.bidNowText,
                    hasAlreadyBid && styles.updateBidText,
                  ]}
                >
                  {hasAlreadyBid ? "Update Quote" : "Bid Now"}
                </Text>
              </View>
            </TouchableOpacity>
          );
      ...
      {/* JOB INFO MODAL */}
      <Modal
        visible={!!selectedJobForBid && !bidModalVisible}
        transparent
        animationType="fade"
      >
        <View style={styles.modalOverlay}>
          <View style={styles.premiumModal}>
            <TouchableOpacity
              ...
              onPress={() => setSelectedJobForBid(null)}
            >
              ...
              <View style={styles.inDepthContainer}>
                <View style={styles.detailSection}>
                  ...
                  <Text style={styles.detailValue}>
                    {selectedJobForBid?.subService || selectedJobForBid?.title}
                  </Text>
                </View>
                <View style={styles.detailSection}>
                ...
                <View style={styles.modalInfoRow}>
                  ...
                  <Text style={styles.infoRowValue}>
                    {formatZAR(
                      selectedJobForBid?.budget ||
                        selectedJobForBid?.customerInitialBid ||
                        0,
                    )}
                  </Text>
                </View>
      ...
      <SafetyBanner
        ...
        onCancel={() => setBannerVisible(false)}
      />
      <HandshakeModal
        visible={pinModalVisible}
        pin={enteredPin}
        setPin={setEnteredPin}
        onVerify={verifyAndStartJob}
        ...
        >
          ...
          <View style={styles.safetyActions}>
            <TouchableOpacity onPress={onCancel}>
              <Text style={styles.cancelLinkText}>Not yet</Text>
            </TouchableOpacity>
    ...
    <Modal visible={visible} transparent animationType="slide">
      <View style={styles.modalOverlay}>
        <View style={styles.premiumModal}>
          ...
          >
            <Text style={styles.modalHeading}>Job Code</Text>
            <TouchableOpacity onPress={onCancel}>
              <X size={20} />
            </TouchableOpacity>
          </View>
          <Text style={styles.modalSubText}>
            Enter the PIN provided by the customer to start.
          </Text>
          <TextInput

...
export default function Marketplace() {
  ...
  const [sortType, setSortType] = useState<"recent" | "price" | "urgency">(
    "recent",
  );
  ...
  const filteredJobs = useMemo(() => {
    ...
    return result.sort((a, b) => {
      ...
      if (sortType === "urgency") {
        ...
        return (
          (weights[b.details?.urgency || "standard"] || 0) -
          (weights[a.details?.urgency || "standard"] || 0)
        );
      }
        ...
        <View style={styles.sortBar}>
          <SortButton
            ...
            icon={
              <Clock
                size={14}
                color={sortType === "recent" ? "#4f46e5" : "#6b7280"}
              />
            }
            onPress={setSortType}
          />
          <SortButton
            ...
            icon={
              <DollarSign
                size={14}
                color={sortType === "price" ? "#4f46e5" : "#6b7280"}
              />
            }
            onPress={setSortType}
          />
          <SortButton
            ...
            icon={
              <Zap
                size={14}
                color={sortType === "urgency" ? "#4f46e5" : "#6b7280"}
              />
            }
            onPress={setSortType}
          />
            ...
            >
              <View style={styles.bookingHeader}>
                <Text style={styles.bookingService}>
                  {item.title || item.subService || "Service Request"}
                </Text>
                <Text style={[styles.statusBadge, styles.statusOpen]}>
                  OPEN
                </Text>
              </View>
            ...
            <Animated.View entering={SlideInBottom} style={styles.floatingCard}>
              ...
              <View style={styles.infoGrid}>
                <InfoItem
                  ...
                  value={
                    ...
                    "Address on acceptance"
                  }
                />
                <InfoItem
                  icon={<Briefcase size={16} color="#4f46e5" />}
                  label="Category"
                  value={selectedJob?.category || "General Service"}
                />
                <InfoItem
                  icon={<DollarSign size={16} color="#4f46e5" />}
                  label="Customer Budget"
                  value={formatCurrency(selectedJob?.budget || 0)}
                />
              </View>
              ...
              <View style={styles.bidSection}>
                ...
                <TouchableOpacity
                  style={[
                    styles.submitBidBtn,
                    (!bidAmount || isSubmitting) && { opacity: 0.7 },
                  ]}
...
function SortButton({ label, type, current, icon, onPress }: any) {
  return (
    <TouchableOpacity style={styles.sortBtn} onPress={() => onPress(type)}>
      {icon}
      <Text style={[styles.sortText, current === type && styles.activeSort]}>
        {label}
      </Text>
    </TouchableOpacity>
  );
